<template>
  <div>
    <div class="gva-search-box">
      <el-form ref="elSearchFormRef" :inline="true" :model="searchInfo" class="demo-form-inline" :rules="searchRule" @keyup.enter="onSubmit">
        ...
        <el-form-item label="tplId字段" prop="tplId">
             ...
             <el-input v-model.number="searchInfo.tplId" placeholder="搜索条件" />
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="number字段" prop="number" width="120" />
        <el-table-column align="left" label="tplId字段" prop="tplId" width="120" />
        <el-table-column align="left" label="0件，1重量，3金额" prop="type" width="120" />
        ...
        <el-table-column align="left" label="操作" fixed="right" min-width="240">
            ...
            <el-button type="primary" link icon="edit" class="table-button" @click="updateBeeShopFreightTplFreeShippingFunc(scope.row)">变更</el-button>
    ...
    <el-drawer destroy-on-close size="800" v-model="dialogFormVisible" :show-close="false" :before-close="closeDialog">
       <template #header>
          ...
          <el-form :model="formData" label-position="top" ref="elFormRef" :rules="rule" label-width="80px">
            ...
            <el-form-item label="tplId字段:"  prop="tplId" >
              <el-input v-model.number="formData.tplId" :clearable="true" placeholder="请输入tplId字段" />
            </el-form-item>
            <el-form-item label="0件，1重量，3金额:"  prop="type" >
              <el-input v-model.number="formData.type" :clearable="true" placeholder="请输入0件，1重量，3金额" />
            </el-form-item>
...
import {
  ...
  deleteBeeShopFreightTplFreeShipping,
  deleteBeeShopFreightTplFreeShippingByIds,
  updateBeeShopFreightTplFreeShipping,
  findBeeShopFreightTplFreeShipping,
  getBeeShopFreightTplFreeShippingList
...
defineOptions({
    name: 'BeeShopFreightTplFreeShipping'
})
...
const getTableData = async() => {
  const table = await getBeeShopFreightTplFreeShippingList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const deleteRow = (row) => {
    ElMessageBox.confirm('确定要删除吗?', '提示', {
        ...
        type: 'warning'
            ...
            deleteBeeShopFreightTplFreeShippingFunc(row)
        })
    }
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteBeeShopFreightTplFreeShippingByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateBeeShopFreightTplFreeShippingFunc = async(row) => {
    const res = await findBeeShopFreightTplFreeShipping({ id: row.id })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteBeeShopFreightTplFreeShippingFunc = async (row) => {
    const res = await deleteBeeShopFreightTplFreeShipping({ id: row.id })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
