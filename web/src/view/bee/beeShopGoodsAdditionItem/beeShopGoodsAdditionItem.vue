  ...
  <div>
    <div class="gva-search-box">
      <el-form ref="elSearchFormRef" :inline="true" :model="searchInfo" class="demo-form-inline" :rules="searchRule" @keyup.enter="onSubmit">
        ...
        <el-form-item label="goodsId字段" prop="goodsId">
            
             <el-input v-model.number="searchInfo.goodsId" placeholder="搜索条件" />

        </el-form-item>
        <el-form-item label="goodsAdditionId字段" prop="goodsAdditionId">
            
             <el-input v-model.number="searchInfo.goodsAdditionId" placeholder="搜索条件" />
    ...
    <div class="gva-table-box">
        ...
        <el-table-column sortable align="left" label="goodsId字段" prop="goodsId" width="120" />
        <el-table-column align="left" label="goodsAdditionId字段" prop="goodsAdditionId" width="120" />
        ...
        <el-table-column align="left" label="操作" fixed="right" min-width="240">
            ...
            <el-button type="primary" link icon="edit" class="table-button" @click="updateBeeShopGoodsAdditionItemFunc(scope.row)">变更</el-button>
...
<!--            </el-form-item>-->
            <el-form-item label="goodsId字段:"  prop="goodsId" >
              <el-input v-model.number="formData.goodsId" :clearable="true" placeholder="请输入goodsId字段" />
            </el-form-item>
            <el-form-item label="goodsAdditionId字段:"  prop="goodsAdditionId" >
              <el-input v-model.number="formData.goodsAdditionId" :clearable="true" placeholder="请输入goodsAdditionId字段" />
            </el-form-item>
...
<script setup>
import {
  createBeeShopGoodsAdditionItem,
  deleteBeeShopGoodsAdditionItem,
  deleteBeeShopGoodsAdditionItemByIds,
  updateBeeShopGoodsAdditionItem,
  findBeeShopGoodsAdditionItem,
  getBeeShopGoodsAdditionItemList
} from '@/api/bee/beeShopGoodsAdditionItem'
...
const getTableData = async() => {
  const table = await getBeeShopGoodsAdditionItemList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const deleteRow = (row) => {
    ElMessageBox.confirm('确定要删除吗?', '提示', {
        ...
        type: 'warning'
            ...
            deleteBeeShopGoodsAdditionItemFunc(row)
        })
    }
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteBeeShopGoodsAdditionItemByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateBeeShopGoodsAdditionItemFunc = async(row) => {
    const res = await findBeeShopGoodsAdditionItem({ id: row.id })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteBeeShopGoodsAdditionItemFunc = async (row) => {
    const res = await deleteBeeShopGoodsAdditionItem({ id: row.id })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
...
const enterDialog = async () => {
     elFormRef.value?.validate( async (valid) => {
             if (!valid) return
              ...
              switch (type.value) {
                case 'create':
                  res = await createBeeShopGoodsAdditionItem(formData.value)
                  break
                case 'update':
                  res = await updateBeeShopGoodsAdditionItem(formData.value)
                  break
                default:
                  res = await createBeeShopGoodsAdditionItem(formData.value)
                  break
