<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="domain字段" prop="domain" width="120" />
        ...
        <el-table-column align="left" label="操作" fixed="right" min-width="240">
            ...
            <el-button type="primary" link icon="edit" class="table-button" @click="updateUserModelsFunc(scope.row)">变更</el-button>
...
<!--            </el-form-item>-->
            <el-form-item label="domain字段:"  prop="domain" >
              <el-input v-model="formData.domain" :clearable="true"  placeholder="请输入domain字段" />
            </el-form-item>
            <el-form-item label="username字段:"  prop="username" >
...
import {
  createUserModels,
  deleteUserModels,
  deleteUserModelsByIds,
  updateUserModels,
  findUserModels,
  getUserModelsList
} from '@/api/bee/userModels'
...
const getTableData = async() => {
  const table = await getUserModelsList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteUserModelsByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateUserModelsFunc = async(row) => {
    const res = await findUserModels({ id: row.id })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteUserModelsFunc = async (row) => {
    const res = await deleteUserModels({ id: row.id })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
...
const enterDialog = async () => {
     elFormRef.value?.validate( async (valid) => {
             if (!valid) return
              ...
              switch (type.value) {
                case 'create':
                  res = await createUserModels(formData.value)
                  break
                case 'update':
                  res = await updateUserModels(formData.value)
                  break
                default:
                  res = await createUserModels(formData.value)
                  break
