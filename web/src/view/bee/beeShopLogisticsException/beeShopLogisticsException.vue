<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column sortable align="left" label="模板详情id" prop="logisticsId" width="120" />
        <el-table-column align="left" label="模板详情id" prop="detailId" width="120" />
        ...
        <el-table-column align="left" label="操作" fixed="right" min-width="240">
            ...
            <el-button type="primary" link icon="edit" class="table-button" @click="updateBeeShopLogisticsExceptionFunc(scope.row)">变更</el-button>
...
<!--            </el-form-item>-->
            <el-form-item label="模板详情id:"  prop="logisticsId" >
              <el-input v-model.number="formData.logisticsId" :clearable="true" placeholder="请输入模板详情id" />
            </el-form-item>
...
import {
  ...
  deleteBeeShopLogisticsException,
  deleteBeeShopLogisticsExceptionByIds,
  updateBeeShopLogisticsException,
  findBeeShopLogisticsException,
  getBeeShopLogisticsExceptionList
...
defineOptions({
    name: 'BeeShopLogisticsException'
})
...
const getTableData = async() => {
  const table = await getBeeShopLogisticsExceptionList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const deleteRow = (row) => {
    ElMessageBox.confirm('确定要删除吗?', '提示', {
        ...
        type: 'warning'
            ...
            deleteBeeShopLogisticsExceptionFunc(row)
        })
    }
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteBeeShopLogisticsExceptionByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateBeeShopLogisticsExceptionFunc = async(row) => {
    const res = await findBeeShopLogisticsException({ id: row.id })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteBeeShopLogisticsExceptionFunc = async (row) => {
    const res = await deleteBeeShopLogisticsException({ id: row.id })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
